...
pub fn build(b: *std.Build) void {
    ...
    const build_dictbuilder = b.option(bool, "ZSTD_BUILD_DICTBUILDER", "Build dictionary builder module") orelse true;
    const build_deprecated = b.option(bool, "ZSTD_BUILD_DEPRECATED", "Build deprecated API module") orelse false;
    const build_legacy = b.option(bool, "ZSTD_BUILD_LEGACY", "Build legacy format support") orelse false;
    ...
    // Freestanding mock headers
    if (is_freestanding) {
        addFreestandingHeaders(b, zstd_lib.root_module);
    ...
    // Zig module for downstream consumers
    ...
    if (is_freestanding) {
        addFreestandingHeaders(b, zstd_module);
    }
    ...
    // Examples (non-freestanding only)
    ...
    // Test step: build and run simple_compression as a smoke test
    ...
    if (!is_freestanding) {
        ...
        const simple_decompression_exe = b.addExecutable(.{
            .name = "test_simple_decompression",
            .root_module = b.createModule(.{
    ...
    if (is_freestanding) {
        addFreestandingHeaders(b, docs_obj.root_module);
    }
...
fn addFreestandingHeaders(b: *std.Build, module: *std.Build.Module) void {
    ...
    module.addIncludePath(write_headers.getDirectory());
}
